...
2. **Reduction**: Events are reduced into a `ReducedContext` containing everything needed for an LLM request
...
- `SetRetryConfigEvent` - Change retry/backoff policy
- `SetProviderConfigEvent` - Change LLM provider or add fallback
...
- `LLMRequestStartedEvent` - Before LLM call
- `LLMRequestCompletedEvent` - After successful LLM call
- `LLMRequestInterruptedEvent` - When request is cancelled (contains partial response)
...
When new user input arrives while an LLM request is in-flight:
1. Cancel the in-flight request
2. Emit `LLMRequestInterruptedEvent` with partial response accumulated so far
3. Start new request with updated context

### Debouncing

When events are added rapidly (e.g., file attachment + message):
- Use "wait for quiet" pattern: wait N ms after last event before starting LLM request
...
- Hooks for before/after LLM requests
...
- Use `Schema.TaggedClass` for events
