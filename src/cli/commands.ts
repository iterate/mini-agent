...
export const remoteOption = Options.text("remote").pipe(
  Options.withDescription("Remote server URL (e.g., http://localhost:3001). If set, uses HTTP client instead of in-process agent."),
  Options.optional
)
...
const runEventStream = (
  ...
  Effect.gen(function*() {
    ...
    // Derive contextName from agentName (same pattern as AgentRegistry)
    ...
    const turnFiber = yield* liveEventsStream.pipe(
    ...
    const sessionEndFiber = yield* liveEventsStream.pipe(
      Stream.takeUntil((e) => e._tag === "SessionEndedEvent"),
      Stream.runForEach((event) => handleEvent(event, options)),
      Effect.fork
    )

    // Note: endSession is not available in AgentService - sessions end naturally
    // For remote mode, we can't end sessions anyway
    // Wait a bit for any final events, then complete
...
const runChat = (options: {
  ...
  Effect.gen(function*() {
    ...
    switch (mode) {
      ...
      case "script": {
        yield* scriptInteractiveLoop(contextName, outputOptions) // contextName is actually agentName here
        break
      }
